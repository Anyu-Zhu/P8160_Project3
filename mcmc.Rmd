...
rest_test_all = mcmc(x = Xlist1, y = Ylist1, ini.mu, ini.bsig, ini.sig, niter = 5000)
...
ids = unique(hurrican$id)
  for (i in 1:20){
    ...
    ##MCMC
    rest_test = mcmc(x = Xlist_train, y = Ylist_train, ini.mu, ini.bsig, ini.sig, niter = 2000)
    beta_coef = colMeans(t(matrix(unlist(rest_test$B), 5, 2000))[1000:2000, ])
