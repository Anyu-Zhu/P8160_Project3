...
for (i in 1:691) {
  hurr_train = Xlist_train[[1]]
  hurr_test = Xlist_test[[1]]
  
  for (j in 9000:10000) {
    beta_coef_all = rest_final$B[[j]][i, ]
    beta_coef = rbind(beta_coef, beta_coef_all)
  }
  ## Coefficients for each hurricane for MCMC
  beta_coef = colMeans(beta_coef)
  ...
  ## RMSE
  RMSE_value = RMSE(pred_test, Ylist_test[[1]]$wind_kt_2)
  
  beta_coef_final = rbind(beta_coef_final, c(RMSE_value, beta_coef, lm_fit_coef))
}
```
