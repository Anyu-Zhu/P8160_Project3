...
rest_test_all = mcmc(x = Xlist[2], y = Ylist[2], ini.mu, ini.bsig, ini.sig, niter = 10000)

all_coef = NULL
for (j in 5000:10000) {
      all_coef = rbind(all_coef, rest_test_all$B[j][[1]])
}
...
ids = unique(hurrican$id)
  for (i in 1:5){
    ...
    ##MCMC
    rest_test = mcmc(x = Xlist_train, y = Ylist_train, ini.mu, ini.bsig, ini.sig, niter = 10000)
    beta_coef = NULL
    ...
    res = rbind(res, c(i, RMSE_value, final_coef, lm_fit_coef))
  }
res
```
