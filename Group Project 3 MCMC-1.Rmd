...
$\epsilon_{i}(t)=Y_{i}(t+6)-\left(\beta_{0, i}+\beta_{1, i} Y_{i}(t)+\beta_{2, i} \Delta_{i, 1}(t)+\beta_{3, i} \Delta_{i, 2}(t)+\beta_{4, i} \Delta_{i, 3}(t)\right) \stackrel{i . i . d}{\sim} N\left(0, \sigma^{2}\right)$

Denote $X_{i} = [1, Y_{i}(t), \Delta_{i, 1}(t), \Delta_{i, 2}(t), \Delta_{i, 3}(t)]$

$$f_{Y_{i}(t+6)}\left(y_{i}(t+6) \mid X_{i}(t), \boldsymbol{\beta}_{i}, \sigma^{2}\right)=\frac{1}{\sqrt{2 \pi} \sigma} \exp \left\{-\frac{1}{2 \sigma^{2}}\left(y_{i}(t+6)-X_{i}(t) \boldsymbol{\beta}_{i}^{\top}\right)^{2}\right\}$$

$$\mathbf{B}= (\boldsymbol{\beta}_{1}^\top,...,\boldsymbol{\beta}_{n}^\top)^\top$$
$$L\left(\mathbf{B}, \sigma^{2}\right) = \prod_{i=1}^{n}\left\{\frac{1}{\sqrt{2 \pi|\sigma I|}} \exp \left(-\frac{1}{2}\left(Y_{i}-X_{i} \boldsymbol{\beta}_{i}^{\top}\right)^{\top}\left(Y_{i}-X_{i} \boldsymbol{\beta}_{i}^{\top}\right)\left(\sigma^{2} I\right)^{-1}  \right)  \right\}$$

$$\pi\left(\mathbf{B} \mid \beta, \Sigma^{-1}\right)=\prod_{i=1}^{n}\left\{\frac{1}{\sqrt{2 \pi|\Sigma|}} \exp \left(-\frac{1}{2}\left(\boldsymbol{\beta}_{i}-\boldsymbol{\beta}\right) \Sigma^{-1}\left(\boldsymbol{\beta}_{i}-\boldsymbol{\beta}\right)^{\top}\right)\right\}$$

$$\Theta=(\mathbf{B}^\top,\boldsymbol{\beta}^\top,\sigma^2, \Sigma)$$
$$\pi\left(\Theta \mid Y\right) \propto L\left(\mathbf{B}, \sigma^{2} I\right) \times \pi\left(\mathbf{B} \mid \boldsymbol{\beta}, \Sigma^{-1}\right) \times \pi(\boldsymbol{\beta}) \times \pi\left(\sigma^{2}\right) \times \pi\left(\Sigma^{-1}\right)$$
